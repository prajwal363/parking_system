...
def init():
    if os.path.exists(DB):
        ...
        print("   Windows : del parking.db")
        print("   Mac/Linux: rm parking.db")
        return
...
-- ─────────────────────────────────────────
CREATE TRIGGER IF NOT EXISTS update_slot_on_booking
...
-- ─────────────────────────────────────────
CREATE TRIGGER IF NOT EXISTS update_slot_on_release
...
-- ─────────────────────────────────────────
CREATE TRIGGER IF NOT EXISTS log_new_booking
AFTER INSERT ON bookings
BEGIN
...
-- ─────────────────────────────────────────
CREATE TRIGGER IF NOT EXISTS log_booking_release
AFTER UPDATE ON bookings
WHEN NEW.status = 'completed'
BEGIN
    INSERT INTO audit_logs (action_type, booking_id, slot_id, vehicle_id, details)
...
-- ─────────────────────────────────────────
-- VIEW 1: Available slots
-- ─────────────────────────────────────────
CREATE VIEW IF NOT EXISTS available_slots_view AS
SELECT slot_id, slot_number, slot_type, floor
FROM parking_slots
WHERE is_available = 1;

-- ─────────────────────────────────────────
-- VIEW 2: Active bookings with full details
-- ─────────────────────────────────────────
...
-- SAMPLE VEHICLES
-- ─────────────────────────────────────────
INSERT OR IGNORE INTO vehicles (owner_name, license_plate, vehicle_type, contact) VALUES
...
""")
    ...
    tables   = c.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name != 'sqlite_sequence'").fetchone()[0]

    conn.close()
    print()
    print("=" * 55)
    print("  parking.db created successfully!")
    print(f"  Tables   : {tables}  (vehicles, parking_slots, bookings, audit_logs)")
